<template>
    <el-row>
      <!-- select -->
    <el-row>
        <el-select v-model="value" placeholder="Quality Name"> 
            <el-option
                v-for="item in names"
                :key="item"
                :label="item"
        ...
        <el-button @click="cardsByQuality(value)" type="primary">
...
export default {
    ...
    methods: {
      cardsByQuality: async function(){
            if(this.value != null && this.value.length > 0){
                this.error = false;
                this.loading = true;
                this.cards = [];
                let info = await ApiService.cardsByQuality(this.value);
                this.cards = info;
        ...
        classNames: async function(){
            ...
            this.names = info.data.qualities;
            this.loading = false;
        },
