<template>
    <el-row>
      <!-- select -->
    <el-row>
        <el-select v-model="value" placeholder="Race Name"> 
            <el-option
                v-for="item in names"
                :key="item"
                :label="item"
        ...
        <el-button @click="cardsByRace(value)" type="primary">
...
export default {
    ...
    methods: {
      cardsByRace: async function(){
            if(this.value != null && this.value.length > 0){
                this.error = false;
                this.loading = true;
                this.cards = [];
                let info = await ApiService.cardsByRace(this.value);
                this.cards = info;
