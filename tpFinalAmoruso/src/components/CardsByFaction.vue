<template>
    <el-row>
      <!-- select -->
    <el-row>
        <el-select v-model="value" placeholder="Faction Name"> 
            <el-option
                v-for="item in names"
                :key="item"
                :label="item"
        ...
        <el-button @click="cardsByFaction(value)" type="primary">
...
export default {
    ...
    methods: {
      cardsByFaction: async function(){
            if(this.value != null && this.value.length > 0){
                this.error = false;
                this.loading = true;
                this.cards = [];
                let info = await ApiService.cardsByFaction(this.value);
                this.cards = info;
